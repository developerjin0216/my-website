...
export const tools: ToolInfo[] = [
  {
    ...
    guide: [
      {
        title: "1단계 — 인스타그램에서 내 정보 다운로드 신청",
        items: [
          "인스타그램 앱 → 프로필 → 메뉴(≡) → 내 활동 → 내 정보 다운로드 (또는 설정 → 계정 센터 → 내 정보 및 권한 → 내 정보 다운로드)",
          "'정보 일부 다운로드' 선택 → 항목에서 '팔로워 및 팔로잉'만 체크하면 파일이 작고 빨리 나옵니다",
          "형식은 반드시 JSON 선택 (HTML 말고), 기간은 '전체'",
          "신청 후 준비되면 알림·메일이 옵니다 — 보통 몇 분에서 몇 시간, ZIP 파일로 다운로드",
        ],
      },
      {
        title: "쓰레드(Threads)에서 받는 법 — 실제 경로",
        items: [
          "쓰레드 앱: 프로필 → 설정(⚙) → 계정 설정 → 내 정보 및 권한 → 내 정보 내보내기 → 내보내기 만들기",
          "'정보 맞춤 설정'에서 쓰레드(나에 관한 정보) 관련 항목만 선택하면 파일이 작고 빨리 나옵니다 — 전부 선택해도 상관없습니다. 어차피 이 사이트는 아무 정보도 저장하지 않으니까요",
          "형식은 JSON을 선택하고 내보내기를 신청하세요",
          "⚠️ 안드로이드 앱에서 다운로드 버튼이 활성화되지 않는 경우가 있습니다 — 그럴 땐 PC 브라우저로 쓰레드(threads.net)에 로그인해 같은 메뉴에서 다운로드하면 됩니다",
          "받은 ZIP을 이 페이지에 그대로 올리면 파일 구조가 같아 자동 인식됩니다",
        ],
      },
      {
        title: "2단계 — 받은 파일 올리기",
