...
// intro는 카테고리 문서(/meme/<카테고리>) 첫머리에 들어가는 해설입니다.
// 용어 나열만 있으면 목록 페이지로 보이고, 용어가 적은 분류(야민정음 3개)는
// 분량 자체가 모자랍니다. 분류가 무엇인지·어떻게 읽어야 하는지를 먼저 씁니다.
export const memeCategories: Record<
  MemeCategoryId,
  { name: string; emoji: string; color: string; desc: string; intro: string[] }
> = {
  recent: {
    name: "요즘 뜨는 말",
    emoji: "🔥",
    color: "#E74C3C",
    desc: "2024~2026 최신 유행어와 밈",
    intro: [
      "유행어는 보통 한 사람의 말버릇에서 시작해 짤과 댓글을 타고 번집니다. 여기 모은 말들은 2024년 이후 온라인에서 자리를 잡아, 지금도 실제 대화에서 쓰이는 것만 골랐습니다.",
      "유행어는 수명이 짧습니다. 한창 돌 때 쓰면 자연스럽지만 한두 해 지나 쓰면 오히려 어색해지는 말이 많습니다. 각 항목의 '사용 팁'에 지금 써도 괜찮은지를 함께 적었습니다.",
      "뜻만 외우기보다 어떤 상황에서 나온 말인지 알아두면 잘못 쓰는 일이 줄어듭니다. 유래가 확실하지 않은 것은 단정하지 않고 알려진 범위만 적었습니다.",
    ],
  },
  abbr: {
    name: "줄임말·신조어",
    emoji: "✂️",
    color: "#3498DB",
    desc: "일상 대화와 SNS에서 쓰는 줄임말",
    intro: [
      "한국어 줄임말은 대개 어절의 첫 글자를 따서 만듭니다. 규칙이 단순해서 새 말이 빠르게 생기고, 맥락을 모르면 전혀 짐작할 수 없다는 것이 특징입니다.",
      "줄임말은 같은 말을 아는 사람들끼리의 표시로도 쓰입니다. 그래서 회사 메일이나 공식 문서에 섞어 쓰면 무례하거나 성의 없어 보일 수 있습니다. 쓰는 자리를 가리는 것이 중요합니다.",
      "여기서는 뜻과 함께 원래 어떤 말을 줄인 것인지를 밝혀 적었습니다. 원말을 알면 처음 보는 줄임말도 대충 읽어낼 수 있습니다.",
    ],
  },
  reaction: {
    name: "리액션·감탄사",
    emoji: "💬",
    color: "#F59E0B",
    desc: "채팅과 댓글에서 쓰는 반응 표현",
    intro: [
      "글로 하는 대화에는 표정도 말투도 없습니다. 그 빈자리를 메우려고 생긴 것이 리액션 표현입니다. 같은 문장이라도 뒤에 무엇을 붙이느냐에 따라 온도가 완전히 달라집니다.",
      "리액션은 뜻보다 강도와 뉘앙스가 중요합니다. 칭찬처럼 보이는 말이 비꼬는 뜻으로 쓰이기도 하고, 반대의 경우도 있습니다. 각 항목에 어떤 자리에서 쓰는 말인지를 적었습니다.",
      "상대와 친밀도가 낮을 때는 리액션 표현을 줄이는 편이 안전합니다. 가벼운 반응이 상황에 따라 성의 없게 읽히기 때문입니다.",
    ],
  },
  yamin: {
    name: "야민정음",
    emoji: "🔤",
    color: "#9B59B6",
    desc: "글자 모양이 비슷한 것끼리 바꿔 쓰는 표기 놀이",
    intro: [
      "야민정음은 뜻이 아니라 글자의 생김새를 가지고 노는 표기 방식입니다. 한글 자모 중 모양이 닮은 것끼리 바꿔 적어, 읽는 사람이 원래 단어를 알아맞히게 만듭니다.",
      "대표적인 원리는 '대'와 '머', '귀'와 '커', '유'와 '윾'처럼 획의 배치가 비슷한 글자를 맞바꾸는 것입니다. 소리가 아니라 모양을 기준으로 삼기 때문에, 소리 내어 읽으면 뜻이 통하지 않습니다.",
      "커뮤니티 안에서는 재치로 받아들여지지만, 검색이 되지 않고 화면 낭독기로도 읽히지 않습니다. 정보를 전달해야 하는 글에는 쓰지 않는 것이 좋습니다.",
      "여기에는 널리 알려져 원래 단어를 대부분 알아보는 표기만 실었습니다. 즉석에서 만들어 쓰는 변형까지 담으면 끝이 없고, 통용되지 않는 표기는 사전에 넣을 이유가 없기 때문입니다.",
    ],
  },
  broadcast: {
    name: "방송·인터넷 밈",
    emoji: "📺",
    color: "#E67E22",
    desc: "예능·영화·인터넷 방송에서 태어난 밈",
    intro: [
      "방송에서 나온 한 장면이나 한마디가 짤로 잘려 돌면서 밈이 됩니다. 원본을 모르면 왜 웃긴지 알 수 없다는 점이 이 분류의 특징입니다.",
      "그래서 각 항목에 어떤 장면에서 나온 말인지를 먼저 적었습니다. 출처를 알고 나면 쓰는 자리도 자연스럽게 잡힙니다.",
      "방송 밈은 원본의 맥락이 따라옵니다. 원래 장면이 누군가를 조롱하는 것이었다면 그 말도 같은 기운을 갖습니다. 가볍게 쓰기 전에 어디서 온 말인지 확인하는 편이 좋습니다.",
    ],
  },
  game: {
    name: "게임에서 온 말",
    emoji: "🎮",
    color: "#22C55E",
    desc: "게임 용어에서 일상어가 된 말",
    intro: [
      "게임에서 쓰던 말이 게임 밖으로 나와 일상어가 되는 일은 꾸준히 있어 왔습니다. 상황을 짧고 정확하게 가리키는 표현이 많아서, 게임을 하지 않는 사람들 사이에서도 자리를 잡았습니다.",
      "다만 원래 게임 안에서의 뜻과 일상에서 쓰이는 뜻이 조금씩 다른 경우가 있습니다. 각 항목에 원래 어떤 상황을 가리키던 말인지를 함께 적었습니다.",
      "게임 용어는 세대와 취향을 드러내는 말이기도 합니다. 모르는 사람 앞에서 설명 없이 쓰면 대화가 끊길 수 있습니다.",
    ],
  },
  fandom: {
    name: "덕질·연애 용어",
    emoji: "💘",
    color: "#E91E63",
    desc: "팬 활동과 연애 관계에서 쓰는 말",
    intro: [
      "무언가를 깊이 좋아하는 일에는 그만의 어휘가 따라붙습니다. 팬 활동에서 생긴 말들은 좋아하는 정도와 관계의 단계를 아주 세밀하게 나눠 부릅니다.",
      "연애 관계를 가리키는 말도 같은 자리에 모았습니다. 관계에 이름을 붙이는 방식이 팬 문화의 어휘와 닮아 있고, 실제로 섞여 쓰이기 때문입니다.",
      "이 분류의 말들은 당사자끼리는 친근하지만 바깥에서 들으면 낯설게 들립니다. 특히 사람을 대상으로 하는 표현은 상대가 불편해할 수 있으니 쓰는 자리를 가려야 합니다.",
    ],
  },
};
